...
function waterPortData(data) {
    ...
    if(stx === '20'){
      let etx = str.substring(str.length-2,str.length);
      if(etx === '0a'){
    	 value_data= Buffer.from(str,'hex');
         value_data = value_data.toString('utf8');
         value_data = value_data.split("    ");
         value_data = value_data[2].trim();
...
function interval_upload(delay){
    ...
    timerId = setInterval(function(){
      if(value_data !=''){ 
        var cnt_obj = {      // flex CNT SDT 
            'wat:wqgi':{
            'ntu': Number(value_data)
            }
        };
        console.log(delay,cnt_obj);
...
function on_mqtt_message_recv(topic, message) {
    ...
    if (topic_arr[1] == 'oneM2M' && topic_arr[2] == 'req' && topic_arr[4] == conf.noti.id) {
        ...
        mqtt_noti_action(jsonObj, function (path_arr, cntObj, rqi, sur) {
            if (cntObj) {
                var rsp_topic = '/oneM2M/resp/' + topic_arr[3] + '/' + topic_arr[4] + '/' + topic_arr[5];

                event.emit('upload', sur, cntObj);
